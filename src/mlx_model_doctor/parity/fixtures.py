...
A fixture supplies the *identical* teacher-forced token-id array every
worker feeds to base/adapter/fused (see
:mod:`mlx_model_doctor.parity.oracle`). Binding it to a
:class:`~mlx_model_doctor.parity.context.TokenizerFingerprint` makes that
binding explicit and checkable: the array only has the meaning it was built
for alongside a tokenizer that produces the exact same fingerprint, so a
fixture built for one tokenizer is never silently fed to a repository with a
different (or unverifiable) one -- the caller compares
...
class FixtureRef:
    ...
    ``max_length`` bounds how many of the fixture's token ids a consumer may
    feed to a model load; ``scored_positions`` are the array indices whose
    per-position argmax should be included when comparing model outputs.
    """
...
def compute_input_digest(token_ids: Sequence[int]) -> str:
    """Return a stable sha256 hex digest of an ordered token-id sequence.

    Order-sensitive and value-sensitive: reordering or changing any id
    changes the digest. Used both to build a ``FixtureRef.input_digest`` and
    to let a caller verify a worker's reported ids against the fixture it
    claims to have used.
    """
...
def token_ids_in_range(token_ids: Sequence[int], *, vocab_size: int) -> bool:
    """Return whether every token id lies in ``[0, vocab_size)``.

    An empty sequence is vacuously in range.
    """
...
_DEFAULT_TOKEN_IDS: tuple[int, ...] = (0, 10, 22, 45, 7, 33, 1)

_DEFAULT_FIXTURE = FixtureRef(
    id=DEFAULT_FIXTURE_ID,
    input_digest=compute_input_digest(_DEFAULT_TOKEN_IDS),
    max_length=len(_DEFAULT_TOKEN_IDS),
    scored_positions=tuple(range(len(_DEFAULT_TOKEN_IDS))),
...
_FIXTURES: dict[str, tuple[FixtureRef, tuple[int, ...]]] = {
    DEFAULT_FIXTURE_ID: (_DEFAULT_FIXTURE, _DEFAULT_TOKEN_IDS),
}


def get_fixture(fixture_id: str) -> tuple[FixtureRef, tuple[int, ...]]:
    """Return the pinned ``(FixtureRef, token_ids)`` pair for ``fixture_id``.
